define([ "animation", "input", "map", "entity", "load" ], function(animation, input, map, entity, load) {
    return load.ready(), {
        walk: function(target, event) {
            ...
            return input.keys.right === !1 && this.
data.direction.right === !0 && (this.counter = 0), input.keys.left === !1 && this.data.direction.left === !0 && (this.counter = 0), !1;
        },
...
locked) {
                ...
                this.data.targetDoor.x = target.x, this.data.targetDoor.y = target.y, this.data.targetDoor.xSpeed = Math.floor((target.x - this.data.tileX) * 32 / 8), this.data.targetDoor.ySpeed = Math.floor((target.y - this.data.tileY) * 32 / 8);
